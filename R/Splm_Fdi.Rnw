...
subset(data_exp, select = -c(dist, bits))
diff(subset(data_exp, select = -c(dist, bits)))

lala<-pdata.frame(data_exp, c("iso2c", "year"))

diff(subset(lala, select = -c(dist, bits, value)))
...
head(data_exp)
fm<-ihs(value)~log(population)+log(gdppc*population)+pol+log(lit)#+log(tradec)+ log(reso*gdppc*population)+log(dist)+log(GDP_po)+pol+fin+eco+log(lit)+as.numeric(year)+I(as.numeric(year)^2)+bits#singular mit der gleichung, wenn ich population und gdppc raushau gehts
...
sarsem<-spml(fm, data_exp, listw=W.list.inv,model="within", effect="twoways", lag=TRUE, spatial.error="non")
