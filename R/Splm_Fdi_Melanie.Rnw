...
#base case
fm<-ihs(value)~log(population)+log(gdp)+log(tradec)+reso+log(dist)+log(gdp_po)+bits+invcost+lit
a <- lm(fm, data)

#base case + trend
fm<-ihs(value)~log(population)+log(gdp)+log(tradec)+reso+log(dist)+log(gdp_po)+bits+invcost+lit+as.numeric(year)
a1 <- lm(fm, data)

#base case + trend²
fm<-ihs(value)~log(population)+log(gdp)+log(tradec)+reso+log(dist)+log(gdp_po)+bits+invcost+lit+as.numeric(year)+I(as.numeric(year)^2)
a2 <- lm(fm, data)
...
#"bsktest"
bsktest(x=fm, data, listw=W.list.inv, test="LMH") #p-value = 0.4334 -> No Random Regional Effects and Spatial autocorrelation
