...
data_exp_cty <- data_exp.p %>%
  left_join(out, by="id") %>%
  mutate(ihspop = ihs(population)) %>%
  mutate(ihsgdp = ihs(gdp))


#-----------------------------------------------------Differencing only GDP, POP - new data set is data_exp_test

data_exp_test <- data_exp %>%
  mutate(gdp = gdppc*population) %>%
  mutate(ggdp = diff(data_exp_test$gdp))
...
fm<-ihs(value)~log(population)+log(gdp)+tradex+reso+log(dist)+log(GDP_po)+bits+pol+fin+eco+lit
...
<<>>=
ggplot(data_exp_cty, aes(x=year, y=gdppc, group=iso2c, colour=iso2c))+
  geom_line()+
  geom_point()+
  scale_x_continuous(breaks=data_exp_cty$year)+
  labs(x="Year", y="X")+
  theme_minimal()
@
...
Testing with non-stationary data

Problem: logs nicht möglich für GDP, Population, gdppc, GDP_po, ihs nicht möglich für gdp, GDP_po

<<>>=
fm<-ihs(value)~gdppc+tradec+reso+log(dist)+bits+pol+fin+eco+lit#+ihs(GDP_po)
...
#Result: LM = 8.3068, df = 1, p-value = 0.00395 -> spatial error dependence

#Test for spatial lag
slmtest(fm,data= data_exp_cty ,listw=W.list.inv, test="lml")
#Result: LM = 5.2536, df = 1, p-value = 0.0219 -> spatial lag dependence
...
bsktest(x=fm, data = data_exp_cty ,listw=W.list.inv, test="LM2") # p-value = 0.00395 -> spatial error correlation
...
#"bsjktest" - Is there serial correlation on top?
bsjktest(x=fm, data = data_exp_year ,listw=W.list.inv, test="J") # p-value = 2.807e-146 -> random effects or serial corr. or spatial dependence in error terms
bsjktest(x=fm, data = data_exp_year ,listw=W.list.inv, test="C.1") # p-value = 0.02766 -> spatial dependence in error terms, sub RE and serial corr.
bsjktest(x=fm, data = data_exp_year ,listw=W.list.inv, test="C.2") # p-value = 1.614e-07 -> serial corr. in error terms, sub RE and spatial dependence
bsjktest(x=fm, data = data_exp_year ,listw=W.list.inv, test="C.3") # p-value = 1 -> no random effects, sub serial corr. and spatial dependence in error terms
@
