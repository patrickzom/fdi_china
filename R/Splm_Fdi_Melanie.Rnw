...
fm<-ihs(value)~log(population)+log(gdp)+tradec+reso+log(dist)+log(gdp_po)+bits+invcost+lit+as.numeric(year)
b <- lm(fm, data)
summary(b)
...
mod3 <- spreml(fm, data, w=W.list.inv, laf=TRUE, errors="semsrre")
summary(mod3)
