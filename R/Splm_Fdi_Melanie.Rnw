...
data_exp.p <- data_exp %>%
  ...
  mutate(gdp_popop = GDP_po / population) %>% #not correct but preleminary
  subset(select= c(iso2c, year, population, gdp, gdppc, GDP_po, gdp_popop)) %>%
  pdata.frame(row.names=TRUE) %>%
  subset(select= -c(iso2c, year))

for(i in 1:ncol(data_exp.p)){
...
out <- data_exp %>%
  subset(select= -c(population, gdppc, GDP_po)) %>%
  mutate(id = paste(data_exp$iso2c, data_exp$year, sep="-"))

data_exp_cty <- out %>%
  left_join(data_exp.p, by="id") %>%
  na.omit() 
...
ggplot(data_exp_cty, aes(x=as.numeric(year), y=ihs(gdppc), group=iso2c, colour=iso2c))+
  geom_line()+
  geom_point()+
  scale_x_continuous(breaks=as.numeric(data_exp_cty$year))+
  labs(x="Year", y="X")+
  theme_minimal()
@
...
<<Testing with data_exp_cty>>=
fm<-ihs(value)~ihs(gdppc)+ihs(gdp_popop)+tradec+reso+log(dist)+bits+pol+fin+eco+lit+as.numeric(year)+I(as.numeric(year)^2)
...
#Test for spatial error
slmtest(fm,data= data_exp_cty ,listw=W.list.inv, test="lme")
#Result: LM = 0.063625, df = 1, p-value = 0.8009 -> no spatial error dependence

#Test for spatial lag
slmtest(fm,data= data_exp_cty ,listw=W.list.inv, test="lml")
#Result: LM = 5.2536, df = 1, p-value = 0.0219 -> no spatial lag dependence

#Test for spatial error given that we have spatial lag dependence
slmtest(fm, data= data_exp_cty, listw=W.list.inv, test="rlml")
#Result: LM = 4.3137, df = 1, p-value = 0.03781 -> spatial error and spatial lag dependence
...
#Use sphtest to perform a spatial Hausmann test: fixed effects?
#Cannot use ML due to Singularity problems
sphtest(x=fm, data=data_exp_cty, listw=W.list.inv, spatial.model="sarar", method="GM")
#Result: chisq = 8.6118, df = 8, p-value = 0.3761 -> Combined model is inconsistent - keep fixed effects model


#"bsktest"
bsktest(x=fm, data = data_exp_cty, listw=W.list.inv, test="LMH") # p-value < 2.2e-16 -> Random Regional Effects and Spatial autocorrelation
bsktest(x=fm, data = data_exp_cty ,listw=W.list.inv, test="LM1") # p-value < 2.2e-16 -> Random effects
bsktest(x=fm, data = data_exp_cty ,listw=W.list.inv, test="LM2") # Problem: p-value = 1.179 !!!!!!
bsktest(x=fm, data = data_exp_cty ,listw=W.list.inv, test="CLMmu") # p-value < 2.2e-16 -> Random regional effects
bsktest(x=fm, data = data_exp_cty ,listw=W.list.inv, test="CLMlambda") # p-value = 0.0005851 -> spatial error correlation

#CONCLUSION: Random effects and spatial error correlation


#"bsjktest" - Is there serial correlation on top?
bsjktest(x=fm, data = data_exp_cty ,listw=W.list.inv, test="J") # p-value = 2.807e-146 -> random effects or serial corr. or spatial dependence in error terms
bsjktest(x=fm, data = data_exp_cty ,listw=W.list.inv, test="C.1") # p-value = 0.02766 -> spatial dependence in error terms, sub RE and serial corr.
bsjktest(x=fm, data = data_exp_cty ,listw=W.list.inv, test="C.2") # p-value = 1.614e-07 -> serial corr. in error terms, sub RE and spatial dependence
bsjktest(x=fm, data = data_exp_cty ,listw=W.list.inv, test="C.3") # p-value = 1 -> no random effects, sub serial corr. and spatial dependence in error terms
@


<<Estimation with data_exp_cty>>=
##- 2. ESTIMATING
# λ : (lambda) : Spatial autoregressive parameter
# σ² ("phi") : Random effects
# ρ : (rho) : Spatial error component
...
fm<-ihs(value)~ihs(gdppc)+ihs(gdp_popop)+tradec+reso+log(dist)+bits+pol+fin+eco+lit+as.numeric(year)

base <- spml(fm, data=data_exp_cty, listw=W.list.inv, model="pooling", lag=FALSE, spatial.error="none")
summary(base)
-2*base$logLik+2*length(data_exp_cty)

b <- lm(fm, data=data_exp_cty)
summary(b)

# Full model including random effects ("model"), lagged dependent ("lag=TRUE") and spatial error ("spatial.error")
sararreb <- spml(fm, data = data_exp_cty, listw=W.list.inv, model="random", lag=TRUE, spatial.error="b")
summary(sararreb)
#Spatial autoregressive parameter, Random effects parameter are significant
sararrekkp <- spml(fm, data = data_exp_cty, listw=W.list.inv, model="random", lag=TRUE, spatial.error="kkp")
summary(sararrekkp)
#Spatial autoregressive parameter, Random effects parameter and Spatial error component are significant
sararreb$logLik < sararrekkp$logLik
...
# Model including random effects ("model"), lagged dependent ("lag=TRUE")
sarreb <- spml(fm, data = data_exp_cty ,listw=W.list.inv, model="random", lag=TRUE, spatial.error="none")
summary(sarreb)
#Spatial autoregressive parameter, Random effects parameter are significant
sarrekkp <- spml(fm, data = data_exp_cty ,listw=W.list.inv, model="random", lag=TRUE, spatial.error="none")
summary(sarrekkp)
#Spatial autoregressive parameter, Random effects parameter are significant
sarreb$logLik = sarrekkp$logLik

# Full model including random effects ("model"), lagged dependent ("lag=TRUE") and spatial error ("spatial.error")
sararre <- spreml(fm, data_exp_cty ,w=W.list.inv, lag = FALSE, errors = "ols")
summary(sararre)
#Serial correlation and spatial autroregressive parameter are significant

sarsem<-spml(fm, data_exp_cty, listw=W.list.inv, lag=TRUE, spatial.error="kkp", model="within", effect="time", method="eigen", quiet=TRUE, zero.policy=NULL, tol.solce=1e-10)
