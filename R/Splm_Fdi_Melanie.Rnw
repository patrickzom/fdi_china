...
fm<-ihs(value)~log(population)+log(gdppc)+tradec+log(reso)+log(dist)+log(GDP_po)+bits+pol+fin+eco+lit+as.numeric(year)+I(as.numeric(year)^2)
...
<<>>=
ggplot(data_exp_year, aes(x=year, y=log(eco), group=iso2c, colour=iso2c))+
  geom_line()+
  geom_point()+
