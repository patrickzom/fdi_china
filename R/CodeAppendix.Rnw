...
We first have a look at the base model with the composite index. Unfortunately the pml-package doesn't come with functions for AIC, therefore we use the adjusted R-Squared to decide on the model.
...
a <- plm(fm, data, model="pooling")
a1 <- plm(fm, data, model="random")
ifelse(r.squared(a, dfcor=TRUE) > r.squared(a1, dfcor=TRUE), "pooling", "random") #pooling

#base case + trend
fm <-ihs(value)~log(population)+log(gdp)+log(tradec)+reso+log(dist)+log(gdp_po)+bits+log(invcost)+lit+as.numeric(year)
a1 <- plm(fm, data, model="pooling")
...
a2 <- plm(fm, data, model="pooling")

#with or without trend?
r.squared(a, dfcor=TRUE) #0.212124
r.squared(a1, dfcor=TRUE) #0.3455652
r.squared(a2, dfcor=TRUE) #0.3522626
#Go for base case + trend²

@

<<Base Model with composite index echo=FALSE, eval=FALSE>>=
#base case
fm <-ihs(value)~log(population)+log(gdp)+log(tradec)+reso+log(dist)+log(gdp_po)+bits+log(invcost)+lit
a <- spml(fm, data, listw=W.list.inv, model="random", lag=FALSE, spatial.error="none")
a1 <- spml(fm, data, listw=W.list.inv, model="pooling", lag=FALSE, spatial.error="none")
#RE or pooling or 
-2*a$logLik+2*length(data) #1883.829
-2*a1$logLik+2*length(data) #1931.992
# Go for random effects

#base case + trend
fm <-ihs(value)~log(population)+log(gdp)+log(tradec)+reso+log(dist)+log(gdp_po)+bits+log(invcost)+lit+as.numeric(year)
a1 <- spml(fm, data, listw=W.list.inv, model="random", lag=FALSE, spatial.error="none")

#base case + trend²
fm <-ihs(value)~log(population)+log(gdp)+log(tradec)+reso+log(dist)+log(gdp_po)+bits+log(invcost)+lit+as.numeric(year)+I(as.numeric(year)^2)
a2 <- spml(fm, data, listw=W.list.inv, model="random", lag=FALSE, spatial.error="none")

#with or without trend?
-2*a$logLik+2*length(data) #1883.829
-2*a1$logLik+2*length(data) #1825.606
-2*a2$logLik+2*length(data) #1819.599
#Go for base case + trend²
...
@
Following the regular approach, i.e. simple OLS-estimation, we would choose a the model with the squared-trend in it. If we ran the spatial tests on the chosen model we wouldn't find any evidence for spatial models (we have done that before but it is not displayed here).
...
#Result: LM = 28.838, df = 1, p-value = 7.868e-08 -> spatial error dependence

#Test for spatial lag
slmtest(fm, data, listw=W.list.inv, test="lml")
#Result: LM = 38.485, df = 1, p-value = 5.517e-10 -> spatial lag dependence

#Error nested in lag?
slmtest(fm, data, listw=W.list.inv, test="rlme")
#Result: LM = 2.2086, df = 1, p-value = 0.1372 -> no error nested in lag

#Lag allowing for spatial error?
slmtest(fm, data, listw=W.list.inv, test="rlml")
#Result: LM = 11.856, df = 1, p-value = 0.0005749 -> lag nested in error

#CONCLUSION: Lag nested in error

#Hausmanntest cannot be conducted due to singularity issue

#"bsktest"
bsktest(x=fm, data, listw=W.list.inv, test="LM1") 
#Result: p-value = 6.52e-10 -> Random effects

bsktest(x=fm, data, listw=W.list.inv, test="LM2") 
#Result: p-value = 7.868e-08 -> Spatial error correlation

bsktest(x=fm, data, listw=W.list.inv, test="CLMmu") 
#Result: p-value = 7.366e-12 -> Random regional effects

bsktest(x=fm, data, listw=W.list.inv, test="CLMlambda") 
#Result: p-value = 0.01289 -> Spatial error correlation given RE

#CONCLUSION: Random effects & Spatial Error


#"bsjktest" - Is there serial correlation on top?

bsjktest(x=fm, data, listw=W.list.inv, test="C.1") 
#Result: p-value = 0.001146 -> spatial dependence in error terms, sub RE and serial corr.

bsjktest(x=fm, data, listw=W.list.inv, test="C.2") 
#Result: p-value = 0.00853 -> serial corr. in error terms, sub RE and spatial dependence

bsjktest(x=fm, data, listw=W.list.inv, test="C.3") 
#Result: p-value = 1 -> weird

#CONCLUSION: SPREML with small serial correlation
...
#with or without trend?
r.squared(a, dfcor=TRUE) #0.2670682
r.squared(a1, dfcor=TRUE) #0.3502069
r.squared(a2, dfcor=TRUE) #0.354379
#Go for base case + trend²

@

<<Base Model with sub-indices, echo=FALSE, eval=FALSE>>=

#base case
fm <-ihs(value)~log(population)+log(gdp)+log(tradec)+reso+log(dist)+log(gdp_po)+bits+lit+fin+eco+pol
b <- spml(fm, data, listw=W.list.inv, model="random", lag=FALSE, spatial.error="none")
b1 <- spml(fm, data, listw=W.list.inv, model="pooling", lag=FALSE, spatial.error="none")
#RE or pooling or 
-2*b$logLik+2*length(data)
-2*b1$logLik+2*length(data)
# Go for random effects
#Indices in log or not?
fm <-ihs(value)~log(population)+log(gdp)+log(tradec)+reso+log(dist)+log(gdp_po)+bits+lit+log(fin)+log(eco)+log(pol)
b1 <- spml(fm, data, listw=W.list.inv, model="random", lag=FALSE, spatial.error="none")
-2*b$logLik+2*length(data)
-2*b1$logLik+2*length(data)
#Go for no-log
...
#base case + trend²
fm <-ihs(value)~log(population)+log(gdp)+log(tradec)+reso+log(dist)+log(gdp_po)+bits+lit+fin+eco+pol+as.numeric(year)+I(as.numeric(year)^2)
b2 <- spml(fm, data, listw=W.list.inv, model="random", lag=FALSE, spatial.error="none")

-2*b$logLik+2*length(data)
-2*b1$logLik+2*length(data)
-2*b2$logLik+2*length(data)
...
#Test for spatial error
slmtest(fm, data, listw=W.list.inv, test="lme")
#Result: LM = 5.3427, df = 1, p-value = 0.02081 -> no spatial error dependence

#Test for spatial lag
slmtest(fm, data,listw=W.list.inv, test="lml")
#Result: LM = 15.747, df = 1, p-value = 7.24e-05 -> Spatial lag dependence

#Error nested in lag?
slmtest(fm, data, listw=W.list.inv, test="rlme")
#Result: LM = 10.683, df = 1, p-value = 0.001081 -> error nested in lag

#Lag allowing for spatial error?
slmtest(fm, data, listw=W.list.inv, test="rlml")
#Result: LM = 21.087, df = 1, p-value = 4.388e-06 -> lag nested in error
...
#"bsktest"
bsktest(x=fm, data, listw=W.list.inv, test="LM1") 
#Result: p-value = 1.226e-07 -> Random effects

bsktest(x=fm, data, listw=W.list.inv, test="LM2") 
#Result: p-value = 0.02081 -> Spatial error correlation

bsktest(x=fm, data, listw=W.list.inv, test="CLMmu") 
#Result: p-value = 3.85e-10 -> Random regional effects

bsktest(x=fm, data, listw=W.list.inv, test="CLMlambda") 
#Result: p-value = 0.01894 -> Spatial error correlation given RE
...
#"bsjktest" - Is there serial correlation on top?
bsjktest(x=fm, data, listw=W.list.inv, test="C.1") 
#Result: p-value = 0.005434 -> spatial dependence in error terms, sub RE and serial corr.
bsjktest(x=fm, data, listw=W.list.inv, test="C.2") 
#Result: p-value = 0.008132 -> serial corr. in error terms, sub RE and spatial dependence
bsjktest(x=fm, data, listw=W.list.inv, test="C.3") 
#Result: p-value = 1 -> weird
...
<<Comparison, echo=FALSE, eval=FALSE>>=
fm <-ihs(value)~log(population)+log(gdp)+log(tradec)+reso+log(dist)+log(gdp_po)+bits+log(invcost)+lit+as.numeric(year)+I(as.numeric(year)^2)
random.composite <- plm(fm, data, model="random")
pooling.composite <- plm(fm, data, model="pooling")

fm <-ihs(value)~log(population)+log(gdp)+log(tradec)+reso+log(dist)+log(gdp_po)+bits+lit+fin+eco+pol+as.numeric(year)+I(as.numeric(year)^2)
random.indices <- plm(fm, data, model="random")
pooling.indices <- plm(fm, data, model="pooling")

stargazer(pooling.composite, pooling.indices, random.composite, random.indices, no.space=TRUE, keep.stat=c("n","rsq","adj.rsq"),model.numbers=TRUE)
...
<<Estimate SPREML, echo=FALSE, eval=FALSE>>=
fm <-ihs(value)~log(population)+log(gdp)+log(tradec)+reso+log(dist)+log(gdp_po)+bits+log(invcost)+lit
spreml.composite <- spreml(fm, data, w=W.list.inv, model="random", lag=TRUE, spatial.error="semsrre")
summary(spreml.composite)
...
<<Country Fixed Effects, echo=FALSE, eval=FALSE>>=
...
<<Time Fixed Effects, echo=FALSE, eval=FALSE>>=
...
<<Comparison BMS-Models, echo=TRUE, eval=TRUE>>=
fm <- ihs(value) ~ log(population)+log(gdp)+log(tradec)+log(dist)+log(gdp_po)+reso+bits+lit+data$`Religious Tensions`+Inflation+GDPGrowth+ForDebt+BudBal
...
stargazer(country.fixed, time.fixed, no.space=TRUE, title="Country and Time Fixed Effects Model after BMS", dep.var.caption="", column.labels=c("Country Fixed Effects", "Time Fixed Effects"))
@
