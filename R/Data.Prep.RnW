...
TEST if GitHub Desktop works

<<>>=
#Working dirctory
setwd("C:/WU/Master/Spatial/Projekt") #Matthias Lokation
setwd("C:/Users/melan/Documents/0686 Spatial Economics/fdi_china") #Melanie Location
getwd()
@
...
GovStab <- POL %>% group_by(.id) %>% filter(.id=="A-Government Stability")
Socio <- POL %>% group_by(.id) %>% filter(.id=="B-Socioeconomic Conditions")
InvProf <- POL %>% group_by(.id) %>% filter(.id=="C-Investment Profile")
IntConf<- POL %>% group_by(.id) %>% filter(.id=="D-Internal Conflict")
ExtConf<- POL %>% group_by(.id) %>% filter(.id=="E-External Conflict")
Corrup<- POL %>% group_by(.id) %>% filter(.id=="F-Corruption")
Milit<- POL %>% group_by(.id) %>% filter(.id=="G-Military in Politics")
Reli<- POL %>% group_by(.id) %>% filter(.id=="H-Religious Tensions")
Ethnic<- POL %>% group_by(.id) %>% filter(.id=="J-Ethnic Tensions")
Democr<- POL %>% group_by(.id) %>% filter(.id=="K-Democratic Accountability")
Bureau<- POL %>% group_by(.id) %>% filter(.id=="L-Bureaucracy Quality")
...
ForDebt <- FIN %>% group_by(.id) %>% filter(.id=="ForDebt")
XRStab <- FIN %>% group_by(.id) %>% filter(.id=="XRStab")
DebtServ <- FIN %>% group_by(.id) %>% filter(.id=="DebtServ")
CAXGS<- FIN %>% group_by(.id) %>% filter(.id=="CAXGS")
IntLiq<- FIN %>% group_by(.id) %>% filter(.id=="IntLiq")


@


Moved upwards because "x" is needed!
<<>>=
...
data_WDI5 <- data_WDI5[data_WDI5$iso2c %in% x, ]
@
...
data_WDI6<-WDI(country = "all", indicator = c("SI.RMT.COST.OB.ZS","SI.RMT.COST.IB.ZS"), start=2003, end=2015)

data_WDI6 <- data_WDI6[data_WDI6$iso2c %in% x, ]
@
