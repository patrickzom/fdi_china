...
data_full <- data_exp %>% left_join(indices, by=c("iso2c","year")) %>% mutate(gdp=gdppc*population) %>% rename.vars(data_full, from="GDP_po", to="gdp_po") 


data_full <- data_full %>% mutate(invest=1/(pol+fin+eco))
...
data_year<-data[order(data$year,match(data$iso2c,rownames(W.dis))),] #stacked by year
...
data_test <- data[,c(34, 3:6, 8, 33, 35:38, 11:32)]
...
modelCd <- bms(countryDat[,1:11], burn=50000, iter=500000, mcmc="bd",user.int=T)
modelTd <- bms(timeDat[,1:11], burn=50000, iter=500000, mcmc="bd",user.int=T)

#Re-Shuffel Order of Variables
data_short <- data_test[,c(1,4,5,7,6,2,3,11,10,9,8)]
...
#so changes in the results
#timeDat: gdp, lit, population
#countryDat: gdppc, populatio, gdp_po

#remove South Africa from the data set
data1 <- data %>% filter(iso2c!="ZA")
data_test1 <- data1[,c(34, 3:6, 8, 33, 35:38, 11:32)] #reduce data set and change columns - removes bits, dist, iso2c, year, gdppc_po
dat.array1 <- panel_unstack(data_test1, tstep=13)
timeDat1 <- panel_stack(demean(dat.array1,3))
countryDat1 <- panel_stack(demean(dat.array1,1))
modelCd1 <- bms(countryDat1[,1:11], burn=50000, iter=500000, mcmc="bd",user.int=T) #populatio, gdp_po; low: tradec, gdppc, lit, reso
modelTd1 <- bms(timeDat1[,1:11], burn=50000, iter=500000, mcmc="bd",user.int=T) #lit, population; low: pol, reso
...
modelTd2 <- bms(timeDat2, burn=50000, iter=500000, mcmc="bd",user.int=T) #lit, population, gdp, Socioeconomic Conditions
#Not sensitive to order or mcmc-algorithm
#countryDat
countryDat2 <- panel_stack(demean(dat.array2,1))
modelCd2 <- bms(countryDat2, burn=50000, iter=500000, mcmc="bd",user.int=T)#population, gdp_po, lit, gdppc, tradec, Religious Tension
...
modelTd3 <- bms(timeDat3, burn=50000, iter=500000, mcmc="bd",user.int=T) #population, lit, pol, IntLiq
countryDat3 <- panel_stack(demean(dat.array3,1))
modelCd3 <- bms(countryDat3, burn=50000, iter=500000, mcmc="bd",user.int=T) #population, gdp_po, tradec, gdppc, lit, XRStab
...
modelTd4 <- bms(timeDat4, burn=50000, iter=500000, mcmc="bd",user.int=T) #lit, population, pol, GDPGrowth
countryDat4 <- panel_stack(demean(dat.array4,1))
modelCd4 <- bms(countryDat4, burn=50000, iter=500000, mcmc="bd",user.int=T) #population*, gdp_po, gdppc, tradec, reso, lit, Inflation*, GDPGrowth, BudBal,
...
#population, inflation, gdp_po, GDPGrowth, Religious Tension, gdppc, lit, BudBal, tradex, reso
...
AO <- data_test[1:(13*3),]
bms5 <- bms(AO, mcmc="enumerate")


#Singularity Problem
SVD <- svd(countryDat)
variance.explained = prop.table(SVD$d^2) 
countrySVD <- SVD$u[,1:2] %*% diag(SVD$d[1:2]) %*% t(SVD$v[,1:2]) #Does not work.

#Next try
rankifremoved <- sapply(1:ncol(timeDat), function (x) qr(timeDat[,-x])$rank)
which(rankifremoved == max(rankifremoved))


bms5 <- bms(countrySVD, mcmc="enumerate", start.value=5)

bms10 <- bms(timeDat, mcmc="enumerate", start.value=10)
